...
export default async function profile({
  ...
  var profileButtonId = null
  if (typeof window !== "undefined") {
    profileButtonId = document.getElementById('myElement')
  }
  if(profileButtonId?.onclick){
    redirect(`/profile/`)
  }
    ...
    <div className={`${poppins.className} flex pt-5 pb-36 container mx-auto items-start gap-5 `}>
      <div className="w-[312px] flex-col flex gap-5 justify-start">
        <div className="border rounded-lg border-solid border-[#e5e7eb] h-auto" >
          ...
          <ul>
            <li id="profile" className={`link ${pathname === '/profile' ? 'bg-[#e5e7eb]  border-[#e5e7eb] pl-0 pr-0 py-2 px-4 font-semibold' : ' border-[#e5e7eb] pl-0 pr-0 py-2 px-4'}`}><Link href="/profile" className="text-sm pt[8px] pb[5px] pl-[10px] pr-[1rem]">Profile</Link></li>
        ...
        {data.User.map((data: { id_user: number; name_user: string; password_user: number, email_user: number; phone_user: number; adress_user: string; point_user: number; level_user: number; showcase_user: number; isDelete_user: number }) => (
          ...
          <div className="border border-[#e5e7eb] rounded-[10px]  w-full relative">
            ...
            {/* profile Name */}
            <ProfileName name={data.name_user}></ProfileName>
            {/* profile level */}
            <ProfileLevel level={data.level_user}></ProfileLevel>
            {/* profile xp bar */}
            <ProfileXpBar percentage={50} xpLeft={0} nextLevel={0}></ProfileXpBar>
            {/* profile stat text */}
            <ProfileStatText point={data.point_user} showcase={data.level_user} level={data.level_user}></ProfileStatText>
          </div>
